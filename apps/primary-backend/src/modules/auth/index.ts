...
export const app = new Elysia({ prefix: '/auth' })
    .post("/sign-up", async ({body}) => {
        const userId = await AuthService.signup(body.email ,body.password) //passing the email and password to request body 
        return {
            id: userId
        }
    },{
        body:AuthModel.signupSchema, //before running the handler validate incoming body against schema 
        response: {
